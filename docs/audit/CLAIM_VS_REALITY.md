...
- **STATUS**: RESOLVED by T022 (spec-collab-fix R004). All three transports (memory, polling, ably) now accept `subscribe(event, cb, { clientId })` and drop messages whose envelope `data.target` is set but does not match the subscriber's clientId — before the callback fires. The shared predicate `_targetAllowsDelivery(data, clientId)` is applied at the polling `_refresh()` dispatch, the ably channel subscribe wrapper, and the memory bus fan-out. The existing R015 userland test at `tests/forge-collab.test.cjs:663` was rewritten to drop the application-layer `if (m.data.target === handle)` guard and still passes on transport-layer filtering alone. New suite at `tests/forge-collab-target-filter.test.cjs` covers all three backends (10/10 green) with one targeted-delivery test plus one broadcast-unchanged test per backend.
...
- **STATUS**: RESOLVED by T024 (spec-collab-fix R005). Ably `cas(name, expected, next)` now returns `Promise<boolean>` and routes the decision through three wire events: the proposer publishes `cas_propose { name, expected, next, reqId, from, ts }`, every node enqueues it, and after a brief election window the authoritative echoer publishes exactly one `cas_won { reqId, winner }`. The echoer is the current lease holder when one exists (admits only proposals whose `expected` matches the live state, ordered by ts asc then clientId asc), or the lowest-clientId participant when nobody holds. The proposer's promise resolves `true` on its own ack, `false` otherwise; a 500 ms timeout resolves `false` with `_internal.lastCasResult.reason === "cas_timeout"`. Winning echoer applies the mutation locally and broadcasts a companion `lease-update` so late joiners converge. New suite at `tests/forge-collab-ably-cas.test.cjs` (4/4 green) covers the two-proposer race under 100 ms latency, the 500 ms timeout path, the holder-of-record first-by-ts rule, and the no-holder lowest-clientId tiebreak. Only the ably transport changed; polling and memory `cas` remain sync-bool so `tryAcquireLease` / `refreshLease` / `releaseLease` and their 153 existing tests are untouched.
