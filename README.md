...
- **Hard caps and shrunk outputs.** A `PostToolUse` filter trims long Bash output (`npm install`, `git diff`, `tsc`, `find`, `curl`) to head + warnings + tail; a 120 s read-tool cache collapses repeat file reads; per-role `max_tokens` caps (2k–16k) bound every Claude turn. Measured against a 13.8k-LOC project (Stacklink/teambrain): **69% reduction on the 5 real Bash outputs an executor produces**, **70% cache hit rate** on a 20-file second-pass read pattern. LLM reasoning-token reduction is not yet benchmarked end-to-end. ([docs/budgets.md](docs/budgets.md))
